...
--print(json.encode(xPlayer))
  ...
  if string.lower(job.name) ~= 'unemployed' and (string.lower(job.name) ~= 'police' or job.ext == nil) then
  ...
  SendNUIMessage({action = "playerId", value = "ID : ".. GetPlayerServerId(PlayerId()) })
...
Citizen.CreateThread(function()
  while true do
     SendNUIMessage({action = "clock", value = GetClockHours() .. ':'.. GetClockMinutes() })
	 Wait(1000)
  end
end)
